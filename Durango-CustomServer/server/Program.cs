...
internal static class Program
{
    ...
    private static void ShutdownSafely(string reason)
    {
        ...
        try
        {
            ...
            _host?.SaveAll();
            _host?.Close();
            if (!SafeSave.FlushPending())
            {
                Console.WriteLine("[boot] ⚠️ หมดเวลารอคิวเซฟลงดิสก์");
            }
    ...
    private static int Main(string[] args)
    {
        ...
        double nextTick = System.Diagnostics.Stopwatch.GetTimestamp();
        long lastSave = 0;
        int loopErrors = 0;
        ...
        while (true)
        {
            ...
            double remainingTicks = nextTick - System.Diagnostics.Stopwatch.GetTimestamp();
            if (remainingTicks > 0)
            {
                int sleepMs = (int)(remainingTicks * 1000.0 / System.Diagnostics.Stopwatch.Frequency);
                if (sleepMs > 0)
                {
                    Thread.Sleep(sleepMs);
                }
            }
            else if (remainingTicks < -(tickIntervalTicks * 4.0))
            {
                // ถ้าหลุดตารางไปหลาย tick (เช่น GC/save ใหญ่) ให้เริ่มนับใหม่
                // แทนการพยายาม "ไล่ทัน" ด้วยการหมุนลูปรัว ๆ
