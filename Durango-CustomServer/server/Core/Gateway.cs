...
public class Gateway
{
    ...
    private const int SessionRateLimit = 30;
    private const long SessionRateWindowMs = 60_000;
    private readonly Dictionary<string, Queue<long>> _sessionRequestsByIp = new();
    private long _nextSessionRateCleanupAt;
    ...
    private void RegisterRoutes()
    {
        ...
        // Status público usado pela tela de seleção de servidores do cliente OffServer.
        // O cliente considera online somente quando recebe HTTP 2xx e "ok": true.
        // O campo "online" é exibido como a quantidade atual de jogadores conectados.
        _webServer.GetRoute["/status"] = delegate(HttpListenerRequest request, Dictionary<string, string> _)
        {
            ...
            return new WebServer.JsonResponse(status.ToString());
        };

        _webServer.GetRoute["/notice"] = (HttpListenerRequest request, Dictionary<string, string> _) =>
            new WebServer.JsonResponse("{}");

        // Status público usado pelo seletor de servidores do cliente OffServer.
        // O cliente considera o servidor online quando recebe HTTP 2xx + "ok": true
        // e exibe o campo "online" como quantidade de jogadores conectados.
        _webServer.GetRoute["/status"] = delegate(HttpListenerRequest request, Dictionary<string, string> _)
        {
            int online = _host.PlayersOnline();
            if (online < 0) online = 0;
        ...
        _webServer.PostRoute["/sessions"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
        {
            ...
            if (!AllowSessionRequest(remoteIp))
            {
                Console.WriteLine($"[gateway] /sessions rate-limit {remoteIp}");
                return new WebServer.JsonResponse(
                    new JObject { ["error"] = "too_many_session_requests" }.ToString(),
                    HttpStatusCode.TooManyRequests);
            }
            ...
            // เซิร์ฟออนไลน์ต้องไม่เชื่อ PlayerContext ที่ส่งมาจาก client
            // /sessions สร้าง context ชั่วคราวของเซิร์ฟเองเสมอ แล้ว /entry จะผูก token
            // กับตัวละครที่มีอยู่จริงและเป็นของ account นี้ ส่วน /players ใช้ context นี้สร้างตัวใหม่
            PlayerContext context = _host.CreateTemporaryContext(null, null, null);
        ...
        _webServer.PostRoute["/players"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
        {
            ...
            // Personagem recém-criado começa no tutorial original de Ancora.
            // A captura do servidor funcional confirma:
            //   TerrainId  = tropical_event_ancora_01
            //   TemplateId = i01ancora180107
            //   Role       = Tutorial
            //   EntryPoint = 54,52 (vem do info.yml do terrain)
            context.PlayerInfo.PlayerLevel = 1;
            context.RegionId = "tropical_event_ancora_01";
            ...
            _worldCtx.Save();
            context.Save();
            Console.WriteLine($"[gateway] /players '{context.PlayerInfo.PlayerName}' job={prototypeId} → {context.EntityId} region={context.RegionId}");
    ...
    private bool AllowSessionRequest(string remoteIp)
    {
        ...
        if (now >= _nextSessionRateCleanupAt)
        {
            _nextSessionRateCleanupAt = now + 5 * 60_000;
            List<string> empty = new();
            foreach (KeyValuePair<string, Queue<long>> pair in _sessionRequestsByIp)
            {
                while (pair.Value.Count > 0 && now - pair.Value.Peek() > SessionRateWindowMs)
                {
                    pair.Value.Dequeue();
                }
                if (pair.Value.Count == 0) empty.Add(pair.Key);
            }
            foreach (string key in empty) _sessionRequestsByIp.Remove(key);
        }

        if (!_sessionRequestsByIp.TryGetValue(remoteIp, out Queue<long> requests))
        {
            requests = new Queue<long>();
            _sessionRequestsByIp[remoteIp] = requests;
        }
        while (requests.Count > 0 && now - requests.Peek() > SessionRateWindowMs)
        {
            requests.Dequeue();
        }
        if (requests.Count >= SessionRateLimit) return false;
