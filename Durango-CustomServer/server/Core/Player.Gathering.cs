...
internal static class CollectibleTable
{
    ...
    // generator id que o cliente conhece -> prototype real que o servidor entrega.
    // Exemplo confirmado na captura original de Ancora:
    //   collectible tree_date / generator "date" -> item prototype "fruit_tropical".
    // Precisamos guardar o id original porque o cliente usa "date" no Collect,
    // mesmo quando o prototype real possui outro nome.
    private static readonly Dictionary<string, string> _generatorIdByCollectiblePrototype =
        new(StringComparer.Ordinal);

    private static string GeneratorAliasKey(string collectibleId, string prototypeId) =>
        collectibleId + "\u001f" + prototypeId;
    ...
    private static GeneratorSpec MakeSpec(string collectibleId, string prototypeId, int order)
    {
        ...
        string generatorId = prototypeId;
        if (_generatorIdByCollectiblePrototype.TryGetValue(
                GeneratorAliasKey(collectibleId, prototypeId),
                out string originalGeneratorId))
        {
            generatorId = originalGeneratorId;
        }

        GeneratorClientData client =
            GeneratorNames().Get(generatorId) ?? GeneratorNames().Get(prototypeId);
    ...
    private static IEnumerable<string> PrototypesFor(string collectibleId)
    {
        ...
        if (string.Equals(collectibleId, "seashell_1", StringComparison.Ordinal))
        {
            Add("clam");
        }
            ...
            foreach (SourceProbeSlot slot in recipe.slots)
            {
                ...
                foreach (SlotSourceInfo info in slot.source_info)
                {
                    ...
                    if (!list.Contains(prototypeId))
                    {
                        ...
                        _generatorIdByCollectiblePrototype.TryAdd(
                            GeneratorAliasKey(info.collectible_id, prototypeId),
                            info.generator_id);
                    }
                }
            }
