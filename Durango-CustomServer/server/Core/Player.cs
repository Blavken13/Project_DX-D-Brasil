...
public partial class Player
{
    ...
    private void HandleTouchMsg(Messages.Touch touch, uint seq)
    {
        ...
        // Jangada do tutorial de Ancora.
        //
        // Ela usa EntityType 9000 e visualmente é um Artifact, mas não pode cair
        // na lógica genérica de construção abaixo. O cliente só abre a janela
        // especial de contribuição quando Touched.Interactions contém
        // Interaction.BuildTutorialBoat (10101).
        //
        // AppearTutorialBoat já entrega ao cliente a sessão "boat" e os contadores
        // log/stem; este Touch apenas informa qual ação deve aparecer ao interagir.
        if (touch.EntityType == 9000 &&
            string.Equals(touch.EntityId, TutorialBoatId, StringComparison.Ordinal) &&
            string.Equals(
                ...
                StringComparison.Ordinal))
        {
            ...
            MergedBlueprint boatBlueprint = BlueprintStore.GetBlueprint(touch.EntityType);
            if (boatBlueprint != null)
            {
                msg.EntityName = boatBlueprint.Name;
            }
            ...
            msg.Interactions = new[]
            {
                (int)(materialsComplete
                    ? Shared.System.Interaction.DepartTutorial
                    : Shared.System.Interaction.BuildTutorialBoat)
            };

            Console.WriteLine(
                $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                $"tocou jangada → " +
                $"{(materialsComplete ? "DepartTutorial" : "BuildTutorialBoat")} " +
