...
public partial class Player
{
    ...
    private const string TutorialBoatProgressStorageKey = "__server_tutorial_boat_v1";

    // Quantidades originais de building/blueprints.json:
    // tutorial_boat -> log x4 + stem x6.
    private const int TutorialBoatRequiredLog = 4;
    private const int TutorialBoatRequiredStem = 6;

    private bool _tutorialBoatVisible;
    private bool _tutorialBoatProgressLoaded;
    private int _tutorialBoatLog;
    private int _tutorialBoatStem;

    private void EnsureTutorialBoatProgressLoaded()
    {
        if (_tutorialBoatProgressLoaded)
        {
            return;
        }

        _tutorialBoatProgressLoaded = true;
        _tutorialBoatLog = 0;
        _tutorialBoatStem = 0;

        if (_context.Storage == null ||
            !_context.Storage.TryGetValue(TutorialBoatProgressStorageKey, out byte[] data) ||
            data == null ||
            data.Length < 8)
        {
            return;
        ...
        _tutorialBoatLog = Math.Clamp(
            BitConverter.ToInt32(data, 0),
            0,
            TutorialBoatRequiredLog);

        _tutorialBoatStem = Math.Clamp(
            BitConverter.ToInt32(data, 4),
            0,
            TutorialBoatRequiredStem);
    }

    private void SaveTutorialBoatProgress()
    {
        _context.Storage ??=
        ...
        Buffer.BlockCopy(
            BitConverter.GetBytes(_tutorialBoatLog),
            0,
            data,
            0,
            4);
        ...
        Buffer.BlockCopy(
            BitConverter.GetBytes(_tutorialBoatStem),
            0,
            data,
            4,
            4);

        _context.Storage[TutorialBoatProgressStorageKey] = data;
        _context.Save();
    }

    private TutorialBoatSessions BuildTutorialBoatSessions()
    {
        EnsureTutorialBoatProgressLoaded();
        ...
        return new TutorialBoatSessions
        {
            Sessions = new[]
            {
                new TutorialSession
                {
                    SessionId = TutorialBoatSessionId,
                    Players = new[] { EntityId },
                    Materials =
                        new System.Collections.Generic.Dictionary<string, int>
                        {
                            ["log"] = _tutorialBoatLog,
                            ["stem"] = _tutorialBoatStem
                        }
                }
            }
        };
    ...
    private static bool IsTutorialBoatMaterial(Item item, string slotId)
    {
        if (string.Equals(slotId, "stem", StringComparison.Ordinal))
        {
            return HasItemTag(item, "stem");
        }

        if (string.Equals(slotId, "log", StringComparison.Ordinal))
        {
            return
                HasItemTag(item, "pillar_thin") ||
                HasItemTag(item, "pillar_normal") ||
                HasItemTag(item, "pillar_thick") ||
                HasItemTag(item, "log");
        }
    ...
    /// <summary>
    /// Mostra a jangada original quando o jogador entra na área do estaleiro de Ancora.
    /// A mensagem e os valores abaixo foram recuperados do tráfego do servidor funcional.
    /// </summary>
    private void SyncTutorialBoatVisibility()
    {
        bool isAncora = string.Equals(
            _world.TerrainInfo?.region_template,
            "i01ancora180107",
            StringComparison.Ordinal);

        const int boatChunkX = 13; // 211 / 16
        const int boatChunkY = 7;  // 118 / 16

        bool shouldBeVisible =
            isAncora &&
            Math.Abs(_centerX - boatChunkX) <= 1 &&
            Math.Abs(_centerY - boatChunkY) <= 1;

        if (!shouldBeVisible)
        {
            if (_tutorialBoatVisible)
            {
                Send(new DisappearEntity { EntityId = "ancora_9000" });
                _tutorialBoatVisible = false;
            }
            return;
        }
        ...
        const string boatId = TutorialBoatId;

        Send(new AppearTutorialBoat
        {
            EntityId = boatId,
            EntityType = 9000,
            IsAlive = true,
            Tile = new Point2(211, 118),
            Size = new Point2(4, 4),
            Height = 4,
            ...
            Display = new ArtifactDisplay
            {
                ...
                Parts = new System.Collections.Generic.Dictionary<string, string>
                {
                    ["common"] = "tutorial_boat"
                }
            },
            ...
            States = new ArtifactState
            {
                ...
                BuildingState = Shared.Building.BuildingState.Occupied,
                Level = 1,
                MaxHealth = 1f
            },

            FounderEntityId = string.Empty,
            Status = BuildTutorialBoatSessions()
        });

        _tutorialBoatVisible = true;

        Console.WriteLine(
            $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} recebeu a jangada ancora_9000 em [211,118]");
    }
    ...
    private void RegisterS02Handlers()
    {
        ...
        _connection.Recv(delegate(ParticipateTutorialBoat msg, PacketHeader header)
        {
            ...
            // ParticipateTutorialBoat não espera OK.
            // O cliente libera/atualiza a interface ao receber o push
            // TutorialBoatSessions.
            Send(BuildTutorialBoatSessions());

            Console.WriteLine(
                $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                $"entrou na sessão da jangada em [{msg.Tile.x},{msg.Tile.y}] " +
                $"log={_tutorialBoatLog}/{TutorialBoatRequiredLog} " +
                $"stem={_tutorialBoatStem}/{TutorialBoatRequiredStem}");
        });
        ...
        _connection.Recv(delegate(PutMaterialsIntoTutorialBoat msg, PacketHeader header)
        {
            ...
            System.Collections.Generic.List<string> removedIds = new();
                     ...
                     in msg.Materials)
            {
                ...
                if (!string.Equals(slotId, "log", StringComparison.Ordinal) &&
                    !string.Equals(slotId, "stem", StringComparison.Ordinal))
                {
                    continue;
                }

                int remaining =
                    string.Equals(slotId, "log", StringComparison.Ordinal)
                        ? TutorialBoatRequiredLog - _tutorialBoatLog
                        : TutorialBoatRequiredStem - _tutorialBoatStem;

                if (remaining <= 0 || pair.Value == null)
                {
                    continue;
                }
                ...
                foreach (string itemId in pair.Value)
                {
                    ...
                    int index = _context.InventoryItems.FindIndex(
                        item => string.Equals(
                            item.Id,
                            itemId,
                            StringComparison.Ordinal));
                    ...
                    if (!IsTutorialBoatMaterial(item, slotId))
                    {
                        Console.WriteLine(
                            $"[tutorial] material recusado na jangada: " +
                            $"{item.Prototype} não serve para slot {slotId}");
            ...
            System.Collections.Generic.List<Pair<string, int>> added =
                new();

            if (addedLog > 0)
            {
                added.Add(new Pair<string, int>("log", addedLog));
            }

            if (addedStem > 0)
            {
                added.Add(new Pair<string, int>("stem", addedStem));
            }

            Send(new TutorialBoatMaterialUpdated
            {
                SessionId = TutorialBoatSessionId,
                PlayerName = _context.PlayerInfo.PlayerName,
            ...
            Console.WriteLine(
                $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                $"contribuiu com a jangada: " +
                $"+{addedLog} log, +{addedStem} stem → " +
                $"log={_tutorialBoatLog}/{TutorialBoatRequiredLog}, " +
                $"stem={_tutorialBoatStem}/{TutorialBoatRequiredStem}");

            if (_tutorialBoatLog >= TutorialBoatRequiredLog &&
                _tutorialBoatStem >= TutorialBoatRequiredStem)
            {
                Console.WriteLine(
                    $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                    "completou os materiais da jangada");
            }
        });
        ...
        _connection.Recv(delegate(DepartTutorial msg, PacketHeader header)
        {
            ...
            const string targetRegionId = "grass_company_safehouse_01";

            bool boatComplete =
                _tutorialBoatLog >= TutorialBoatRequiredLog &&
                _tutorialBoatStem >= TutorialBoatRequiredStem;

            bool targetReady =
                RegionCatalog.TryGet(targetRegionId, out Region targetRegion) &&
                string.Equals(
                    targetRegion.TemplateId,
                    "sh05tr180130",
                    StringComparison.Ordinal);
                ...
                targetReady)
            {
                Console.WriteLine(
                    $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                    $"DepartTutorial -> DepartTutorialReady " +
                    $"target={targetRegionId} offset=0");
        ...
        _connection.Recv(delegate(DepartTutorialFor msg, PacketHeader header)
        {
            const string targetRegionId = "grass_company_safehouse_01";
            ...
            if (!valid)
            {
                Console.WriteLine(
                    $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                    $"DepartTutorialFor rejeitado: " +
                    $"target='{msg.TargetRegionId}' " +
                    $"offset={msg.EntryPointOffset} " +
                    $"log={_tutorialBoatLog}/{TutorialBoatRequiredLog} " +
                    $"stem={_tutorialBoatStem}/{TutorialBoatRequiredStem}");
            ...
            _context.RegionId = targetRegionId;
            ...
            Console.WriteLine(
                $"[tutorial] {EntityId[..Math.Min(8, EntityId.Length)]} " +
                $"DepartTutorialFor confirmado -> {targetRegionId}; " +
                "estado salvo; enviando Emigrated");

            Send(new Emigrated
            {
                Type = Shared.Teleport.TeleportType.Unknown
            });
        });
