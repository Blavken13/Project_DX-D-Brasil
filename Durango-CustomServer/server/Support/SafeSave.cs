...
public static class SafeSave
{
    ...
    private static readonly Dictionary<string, PendingWrite> PendingWrites =
        new(StringComparer.OrdinalIgnoreCase);
    private static readonly HashSet<string> QueuedPaths =
        new(StringComparer.OrdinalIgnoreCase);
    private static readonly AutoResetEvent WorkReady = new(false);
    private static readonly ManualResetEventSlim QueueIdle = new(true);
    ...
    static SafeSave()
    {
        ...
        {
            IsBackground = true,
            Name = "Durango Save Worker"
        };
        worker.Start();
    }

    /// <summary>
    /// เตรียม snapshot แล้วส่งเฉพาะงาน I/O ไปเธรดเซฟ
    /// งานเดิมของเกมยัง serialize บน main thread จึงไม่อ่าน state พร้อมกับที่เกมกำลังแก้
    /// แต่ FileStream/Flush/File.Replace ไม่บล็อก tick อีกต่อไป
    /// </summary>
    public static bool QueueAtomic(string path, byte[] data, string what)
    {
        if (string.IsNullOrEmpty(path) || data == null || data.Length == 0)
        {
            Console.WriteLine($"[{what}] ⚠️ ข้อมูลที่จะเซฟว่างเปล่า — ไม่เข้าคิว path={path}");
            return false;
        }

        lock (QueueSync)
        {
            PendingWrites[path] = new PendingWrite { Data = data, What = what };
    ...
    /// <summary>รอให้งานเซฟที่เข้าคิวเขียนลงดิสก์หมด — ใช้ก่อนปิด process</summary>
    public static bool FlushPending(int timeoutMs = 30000) => QueueIdle.Wait(timeoutMs);
        ...
        while (true)
        {
            ...
            lock (QueueSync)
            {
                if (PendingPaths.Count > 0)
                {
                    ...
                    if (PendingWrites.TryGetValue(path, out write))
                    {
            ...
            try
            {
                if (WriteAtomic(path, write.Data, write.What))
                {
            ...
            finally
            {
                ...
                {
                    ActiveWrites--;
                    if (PendingPaths.Count == 0 && PendingWrites.Count == 0 && ActiveWrites == 0)
                    {
                        QueueIdle.Set();
                    }
                }
