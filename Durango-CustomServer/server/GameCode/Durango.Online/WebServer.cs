...
public class WebServer
{
	...
	private void KnockListenerCallback(IAsyncResult result)
	{
		...
		try
		{
			...
			if (text.StartsWith("Knock", StringComparison.Ordinal))
			{
		...
		while (linkedListNode != null)
		{
			...
			try
			{
				...
				try
				{
					...
					if (adminPath != null &&
					    (adminPath.StartsWith("/admin", StringComparison.OrdinalIgnoreCase) ||
					     adminPath.Equals("/health", StringComparison.OrdinalIgnoreCase)))
					{
						key.Response.Headers["Access-Control-Allow-Origin"] = "*";
						key.Response.Headers["Access-Control-Allow-Methods"] = "GET, POST, OPTIONS";
						key.Response.Headers["Access-Control-Allow-Headers"] = "Content-Type, X-Admin-Token";
					}
				}
				...
				if (directLength.HasValue)
				{
					...
					// คิวเต็ม — เก็บ response นี้รอ tick ถัดไป ห้ามส่งบน game loop
					// เพราะ client ช้าเพียงคนเดียวจะบล็อก World.Process ของทุกคน
					linkedListNode = linkedListNode.Next;
					continue;
				}
				else
	...
	private Response Process(HttpListenerContext context)
	{
		...
		if (context.Request.HttpMethod.Equals("OPTIONS", StringComparison.OrdinalIgnoreCase))
		{
			...
			context.Response.Headers["Access-Control-Allow-Headers"] = "Content-Type, X-Admin-Token";
			context.Response.Headers["Access-Control-Max-Age"] = "600";
			return new TextResponse("text/plain", string.Empty, HttpStatusCode.NoContent);
		}
		bool flag = false;
