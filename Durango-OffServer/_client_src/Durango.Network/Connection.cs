...
public class Connection
{
	...
	public Connection(bool timeSynchronize = false)
	{
		...
		OffServerLink.IsAdmin = false;
		On(delegate(CheatFlags msg, PacketHeader header)
		{
			OffServerLink.SetAdminFlags(msg.Flags);
		});
	}
	...
	public ReplyMessageHandlerRegistrar Send<T>(T msg, bool noReply = false, uint replyOf = 0u)
	{
		...
		if (_hookList != null)
		{
			for (int num = _hookList.Count - 1; num >= 0; num--)
			{
				if (_hookList[num].HookSendingMessage(this, _sequenceNumber, msg, noReply, replyOf))
				{
					...
					return new ReplyMessageHandlerRegistrar(this, seq);
				}
			}
		}
		if (!Connected())
		...
		uint seq2 = _sequenceNumber++;
		try
		{
			int num2 = Packet.SerializeMsg(GetPredictedServerTime(), seq2, replyOf, msg, SendBuffer, _sendBufferSize, _packingBuffer, _compressingBuffer, _messagePacker);
			_sendBufferSize += num2;
		}
		catch (Exception)
		{
			Close();
		...
		return new ReplyMessageHandlerRegistrar(this, seq2);
	}
	...
	private void HandleMsg(Packet packet)
	{
		...
		packetHandler?.Invoke(packet.Header, packet.Payload, packet.PayloadOffset);
