...
public abstract class UIBasicSprite : UIWidget
{
	...
	public Vector2 GetTileSize()
	{
		if ((mTile.on || type == Type.Tiled) && type != Type.Filled)
		{
			...
			return new Vector2
			{
				x = ((type != Type.Tiled && mTile.width > 0) ? ((float)mTile.width) : ((!(texture == null)) ? (mOuterUV.width * (float)texture.width) : ((float)base.width))),
				y = ((type != Type.Tiled && mTile.height > 0) ? ((float)mTile.height) : ((!(texture == null)) ? (mOuterUV.height * (float)texture.height) : ((float)base.height)))
			} * pixelSize;
		}
		Vector4 vector = drawingDimensions;
		return new Vector2(vector.z - vector.x, vector.w - vector.y);
	}
