...
public class ClientInteractionToDo : SelectableObject
{
	...
	public static ClientInteractionToDo Attach(GameObject host, string entityId, string targetName, string actionName, string todo, string icon = "")
	{
		ClientInteractionToDo obj = host.GetComponent<ClientInteractionToDo>() ?? host.AddComponent<ClientInteractionToDo>();
		obj.SetEntityId(entityId);
		obj._targetName = targetName;
		obj._actionList.Clear();
		obj._actionList.Add(new ActionElem
		{
			Name = actionName,
			ToDo = todo,
			Icon = icon,
	...
	public override bool MenuClicked(GameObject target, InteractionMenuData menu)
	{
		foreach (ActionElem action in _actionList)
		{
			...
			bool hasMotion = !string.IsNullOrEmpty(action.MotionName);
			if (hasMotion)
			{
