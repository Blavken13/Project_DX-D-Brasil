...
using Durango.Localization.Overrides;
...
public static class LocalizeSystem
{
	...
	public static string SetLocale(string locale)
	{
		...
		try
		{
			byte[] offlineCatalogBytes = LanguageOverride.GetOfflineCatalogBytes(locale);
			if (offlineCatalogBytes != null)
			{
				using MemoryStream moFileStream = new MemoryStream(offlineCatalogBytes);
