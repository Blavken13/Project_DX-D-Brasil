...
public struct RemoveFavoriteRegionOwners
{
	...
	public static RemoveFavoriteRegionOwners Unpack(Unpacker unpacker)
	{
		...
		RemoveFavoriteRegionOwners result = new RemoveFavoriteRegionOwners
		{
			EntityIds = new string[num]
		};
		for (int i = 0; i < num; i++)
		{
			unpacker.Read();
	...
	public override string ToString()
	{
		...
		object[] args = entityIds;
		return string.Format("<RemoveFavoriteRegionOwners EntityIds={0}>", args);
	}
}
