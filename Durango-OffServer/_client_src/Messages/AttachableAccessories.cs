...
public struct AttachableAccessories
{
	...
	public static AttachableAccessories Unpack(Unpacker unpacker)
	{
		...
		AttachableAccessories result = new AttachableAccessories
		{
			Accessories = new string[num]
		};
		for (int i = 0; i < num; i++)
		{
			unpacker.Read();
	...
	public override string ToString()
	{
		...
		object[] args = accessories;
		return string.Format("<AttachableAccessories Accessories={0}>", args);
	}
}
