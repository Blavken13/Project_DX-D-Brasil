	...
	public void Recenter()
	{
		...
		if (nextPageThreshold > 0f && UICamera.currentTouch != null && mCenteredObject != null && mCenteredObject.transform == ((list == null) ? transform.GetChild(index) : list[index]))
		{
			...
			if (Mathf.Abs(num7) > nextPageThreshold)
			{
				...
				{
					target = ((list != null) ? ((num2 > 0) ? list[num2 - 1] : ((!(GetComponent<UIWrapContent>() == null)) ? list[list.Count - 1] : list[0])) : ((num2 > 0) ? transform.GetChild(num2 - 1) : ((!(GetComponent<UIWrapContent>() == null)) ? transform.GetChild(transform.childCount - 1) : transform.GetChild(0))));
				}
				else if (num7 < 0f - nextPageThreshold)
				{
					target = ((list != null) ? ((num2 < list.Count - 1) ? list[num2 + 1] : ((!(GetComponent<UIWrapContent>() == null)) ? list[0] : list[list.Count - 1])) : ((num2 < transform.childCount - 1) ? transform.GetChild(num2 + 1) : ((!(GetComponent<UIWrapContent>() == null)) ? transform.GetChild(0) : transform.GetChild(transform.childCount - 1))));
				}
			}
		}
