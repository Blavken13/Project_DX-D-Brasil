...
public class PersonalMaps : Singleton<PersonalMaps>
{
	...
	private IEnumerator CoCapture(Point2 minTile, Point2 maxTile, Action<float?> onProgress, Action<MemoryStream> onResult)
	{
		...
		using (BitmapCaptor bitmapCaptor = new BitmapCaptor(mainCamera))
		{
			...
			int num = bitmapCaptor.Width * xTotalStep;
			int height = bitmapCaptor.Height * yTotalStep;
			int bitmapWidth = num;
			int bitmapHeight = bitmapCaptor.Height;
			byte[] bitmapBuffer = new byte[bitmapWidth * bitmapHeight * 3];
			JpegCompressor jpegCompressor = JpegCompressor.Create(num, height, 90);
			for (int y = 0; y < yTotalStep; y++)
			{
				...
				for (int x = 0; x < xTotalStep; x++)
				{
					...
					Vector3 worldPosition = worldPosStart + x * worldIntervalX + posY;
					...
					{
						yield return CoSetPositionAndWaiting(Util.WorldPositionToClientPosition(worldPosition));
					...
					{
						int num2;
						currentProgress = (num2 = currentProgress + 1);
						onProgress((float)num2 / (float)totalProgress);
					}
				}
