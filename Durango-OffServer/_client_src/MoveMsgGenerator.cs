...
public class MoveMsgGenerator
{
	...
	private void SendMoveMessage()
	{
		if (_movements.Count != 0)
		{
			Move msg = new Move
			{
				...
				Movements = CompactMovement().ToArray()
			};
			_movements.Clear();
	...
	private IEnumerable<Location> CompactPath(IEnumerable<Location> path)
	{
		...
		foreach (Location loc in path)
		{
			...
			GetLocationPathArguments(bl, loc, out var deltaPos, out var deltaYaw, out var deltaHeight);
			if (!prev.HasValue)
			{
				posVelocity = deltaPos;
				yawVelocity = deltaYaw;
				heightVelocity = deltaHeight;
				prev = loc;
			}
			else if (IsSimilarPathArguments(posVelocity, deltaPos, yawVelocity, deltaYaw, heightVelocity, deltaHeight))
			{
				prev = loc;
			}
			else
	...
	private IEnumerable<Movement> CompactMovement()
	{
		...
		for (int i = 0; i <= _movements.Count; i++)
		{
			...
			{
				if (i < _movements.Count && IsSimilarMovement(_movements[i], _movements[num]))
				{
					continue;
				}
				...
				for (int j = num; j < i; j++)
				{
					Location[] path = _movements[j].Path;
					if (path != null)
					{
						IEnumerable<Location> enumerable2 = ((enumerable != null) ? enumerable.Concat(path) : path);
						enumerable = enumerable2;
					}
				}
				Movement movement = _movements[num];
				movement.Path = CompactPath(enumerable).ToArray();
