...
public class ModularArtifact : EnterableArtifact
{
	...
	private static void UpdateRoof(ModelComponent models, string roofModel, string texture, Point2 size, int stories, Vector3 offset)
	{
		...
		bool num3 = num != size.y;
		Vector2 directionPivot = ArtifactUtil.GetDirectionPivot(num3 ? Direction.SouthEast : Direction.SouthWest);
		Vector3 vector = ArtifactUtil.DirectionToAngle(num3 ? Direction.SouthEast : Direction.SouthWest);
		...
		Vector3 vector4 = (num3 ? Vector3.forward : Vector3.right);
	...
	public IEnumerable<ModelComponent.IModel> GetWallModels(int floor, Point2 tile, Direction dir)
	{
		...
		{
			ModelComponent.IModel model = category.GetChild(i).GetModel(key);
			if (!model.IsNull())
			{
				yield return model;
			}
