...
public class CraftSystem : GameSystem<CraftSystem>
{
	...
	public void DoNextCraftQueue()
	{
		if (_craftQueue.Count != 0)
		{
			...
			CraftQueueItem craftQueueItem3 = craftQueueItem;
			Craft msg = new Craft
			{
				RecipeId = craftQueueItem3.Recipe.Id,
				Materials = craftQueueItem3.MakeMaterialIds(),
				ToolItemId = ((craftQueueItem3.Tool != null) ? craftQueueItem3.Tool.Id : string.Empty),
				Workbench = ((!(craftQueueItem3.Workbench == null)) ? new PropKey?(craftQueueItem3.Workbench.GetPropKey()) : ((PropKey?)null))
			};
