...
public class VehicleAirBalloon : VehicleProp
{
	...
	private bool IsProhibitedLandingArea()
	{
		...
		if (Collisions.TryCapsuleCast(Collisions.CreateCollisionParam(position, Vector3.zero), out var raycastHit) != RayCastResult.Pass && IsCollidable(raycastHit))
		{
			return true;
		}
	...
	private void SetHoveringMode(bool on)
	{
		if (base.IsLocalPlayers)
		{
			...
			if (on)
			{
				UIManager.FindScript<ScreenCaptureGroup>().ToggleScreenEffect(Durango.Render.Screen.ScreenCapture.EffectEnum.Tilt, on: false);
			}
			else
			{
