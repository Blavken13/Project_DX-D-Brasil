	...
	public void ProcessText(TextBuilder builder = null)
	{
		...
		using (disposable)
		{
			...
			if (fontSize > 0)
			{
				...
				int num3 = builder.ProcessText(minSize: (overflow != Overflow.ShrinkContent && (uint)(overflow - 2) > 1u) ? fontSize : Mathf.Max(2, mMinFontSize), tokens: tokens, result: _processedTokens, printedSize: out mCalculatedSize, useEllipsis: mOverflowEllipsis, wrapAlways: mWrapAlways);
