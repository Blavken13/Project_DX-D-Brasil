...
public class PlayGuideSystem : GameSystem<PlayGuideSystem>
{
	...
	public string CurrentEventName
	{
		get
		{
			if (_currentEvent != null && _currentEvent != BlankEvent)
			{
	...
	public List<string> ActiveFlowsInfo()
	{
		List<string> list = new List<string>();
		for (int i = 0; i < _flowStacks.Count; i++)
		{
			FlowStack flowStack = _flowStacks[i];
			if (flowStack.Progressed)
			{
				list.Add(flowStack.Name + ":" + (flowStack.GetCurrent() ?? (flowStack.Completed ? "(done)" : "")));
			}
		}
		return list;
	}
	...
	{
		return new SetStorageItem
		{
			Key = key,
			Value = Json.WriteToBytes(value)
		};
	}
	...
	public void Initialize(Role type, [CanBeNull] Dictionary<string, byte[]> storage, bool myPersonalRegion)
	{
		ClearAll();
		Initialized = true;
		NpcSpawner.OnRegionInitialized();
	...
	private void SetCurrentEvent([NotNull] GuideEvent current)
	{
		...
		Command.OnEventStarted(_currentEvent.Name);
