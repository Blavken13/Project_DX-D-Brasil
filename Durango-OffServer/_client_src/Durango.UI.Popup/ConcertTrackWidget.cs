	...
	private void SetPlayer(PlayerInfo info)
	{
		...
		if (info == null || !info.Valid)
		{
			if (_track != null && !string.IsNullOrEmpty(_track.PlayerId))
			{
				...
				for (int j = 0; j < isMeObjects.Length; j++)
				{
					isMeObjects[j].gameObject.SetActive(value: false);
				}
			}
			else
			{
				...
				_infoLabel.text = string.Empty;
				isMeObjects = _emptyPlayerObject;
				for (int k = 0; k < isMeObjects.Length; k++)
				{
					isMeObjects[k].gameObject.SetActive(value: true);
				}
			}
		}
		else
		{
			...
			isMeObjects = _emptyPlayerObject;
			for (int l = 0; l < isMeObjects.Length; l++)
			{
				isMeObjects[l].gameObject.SetActive(value: false);
			}
