...
public class Gateway
{
	...
	public Gateway(GameServer gameServer, WorldContext worldCtx, PlayerContext playerCtx)
	{
		...
		_webServer.GetRoute["/knock"] = delegate(HttpListenerRequest request, Dictionary<string, string> postData)
		{
			...
			string text = "live";
			string text2 = request.QueryString.Get("platform");
			text2 = ((text2 != null && text2.Equals("iPhonePlayer", StringComparison.OrdinalIgnoreCase)) ? "ios" : ((text2 == null || !text2.Equals("Android", StringComparison.OrdinalIgnoreCase)) ? "windows" : "android"));
			jObject["assetbundle_index_url"] = "http://assetbundles.k.nexon.com/" + text + "/" + text2 + "/Info.5.2.1.json";
			jObject["assetbundle_url_root"] = "http://durango-assetbundles.akamaized.net/" + text + "/" + text2 + "/";
