...
public class BoneLookAtTarget : MonoBehaviour
{
	private struct LookAtCoord
	{
		...
		{
			LookAtCoord result = new LookAtCoord
			{
				_height = targetPos.y
			};
			targetPos.y = 0f;
		...
		{
			return new LookAtCoord
			{
				_height = Mathf.Lerp(v1._height, v2._height, t),
				_distance2D = Mathf.Lerp(v1._distance2D, v2._distance2D, t),
				_yawWorld = Maths.NormalizeAngDeg(Mathf.Lerp(Maths.NormalizeAngDeg(v1._yawWorld - myYawWorld), Maths.NormalizeAngDeg(v2._yawWorld - myYawWorld), t) + myYawWorld),
				_valid = true
			};
		}
	}
