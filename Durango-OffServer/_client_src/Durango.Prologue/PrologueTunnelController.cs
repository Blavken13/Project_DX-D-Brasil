...
public class PrologueTunnelController : Singleton<PrologueTunnelController>
{
	...
	private IEnumerator coStartTunnelEffect()
	{
		...
		{
			float time = _timeLine.Evaluate(Time.time - beginTime) / (float)_numColorCorrectionLevel;
			_colorCorrection.Time = time;
			yield return null;
		}
	}
	...
	private IEnumerator coLightning()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			float duration = Random.Range(_lightningBrightDurationMin, _lightningBrightDurationMax);
			scrollBackgroundController.SetTreeVisible(bNormal: false, bThunder: true);
			SetLightningLitSphere(bLightning: true);
			yield return StartCoroutine(coLightningMeshFading(0f, _lightningIntensity, duration));
			SetColorCorrectionFromCC_ID(_lightningNormalCC_ID);
			_colorCorrection.NightTimeOverride = _nightCurveAfterTunnel;
			float duration2 = Random.Range(_lightningDarkDuraionMin, _lightningDarkDuraionMax);
			scrollBackgroundController.SetTreeVisible(bNormal: true, bThunder: false);
			SetLightningLitSphere(bLightning: false);
			yield return StartCoroutine(coLightningMeshFading(_lightningIntensity, 0f, duration2));
		}
	}
	...
	private IEnumerator coLightningMeshFading(float intensityFrom, float intensityTo, float duration)
	{
		...
		while (Time.time < endTime)
		{
			...
			float time = 1f - num;
			float thunderMeshIntensity = Mathf.Lerp(intensityFrom, intensityTo, _lightningIntensityCurve.Evaluate(time));
