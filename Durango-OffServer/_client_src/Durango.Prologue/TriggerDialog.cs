...
public class TriggerDialog : TriggerOnce
{
	...
	private IEnumerator BeginDialog()
	{
		...
		for (int i = 0; i < count; i++)
		{
			DialogElem dialogElem = _dialogs[i];
			TalkerInfo talker = GetTalker(dialogElem.talkerIndex);
			...
			{
				talker.MotionPlayable.CrossFade(dialogElem.motion, 0.5f, (talker.MotionPlayable.GetWrapMode(dialogElem.motion) & WrapMode.Loop) > WrapMode.Default);
			}
			Singleton<PrologueManager>.Instance().NPCFloatingGroup.ShowChatMsg(talker.BubbleTalkable, ConditionalText.Format(dialogElem.text), this);
