...
public class ScalableGaugeAddon : MonoBehaviour
{
	...
	private IEnumerator AnimatedGaugeSequence(float ratio, bool isHorizontal)
	{
		...
		{
			float t = time / _aniTime;
			if (!isHorizontal)
			{
				_gaugeContent.height = (int)Mathf.Lerp(_gaugeContent.height, (float)Widget.height * ratio, t);
			}
			else
			{
				_gaugeContent.width = (int)Mathf.Lerp(_gaugeContent.width, (float)Widget.width * ratio, t);
			}
			yield return null;
		}
	}
