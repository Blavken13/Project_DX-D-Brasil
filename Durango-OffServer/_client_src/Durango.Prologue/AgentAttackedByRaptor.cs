...
public class AgentAttackedByRaptor : MonoBehaviour
{
	...
	{
		GameObject gameObject = KUtility.FindObjectByName(Singleton<PrologueManager>.Instance().TriggersGroup, _targetDoorName, includeInactive: true);
		if ((bool)gameObject)
		{
			_targetDoor = gameObject.GetComponent<TriggerDoorController>();
		}
