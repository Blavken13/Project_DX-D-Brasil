...
public class NpcAIDog : StateBasedAI<NpcAIDog.State>
{
	...
	private IEnumerator NormalDoing()
	{
		...
		else
		{
			...
			{
				base.CurState = stateCandidate.NextState;
			}
		}
		yield return WaitForOneSecond;
	}
	...
	{
		float ang = Maths.CalcYawWithTarget(base.MasterPos, base.transform.position);
		float duration = UnityEngine.Random.Range(_barkDurationMin, _barkDurationMax);
		if (Maths.DistanceAngDeg(ang, TargetAnimal.CurrentYaw) > _turnMotionAcivateAngle)
		{
		...
		while (Time.time < endTime)
		{
			...
			ang = Maths.CalcYawWithTarget(base.MasterPos, base.transform.position);
			TargetAnimal.TurnToYaw(ang, bSnap: false);
			yield return null;
		}
		base.CurState = State.Normal;
	}
	...
	private IEnumerator CoMoveTo(Func<Vector3> funcTargetPos, Func<bool> funcTransition, string moveMotion, float moveSpeed, bool endAtReached = false, float fadeInTime = 0.1f)
	{
		...
		{
			float ang = Maths.CalcYawWithTarget(funcTargetPos(), base.transform.position);
			if (!isMoving && Maths.DistanceAngDeg(ang, TargetAnimal.CurrentYaw) > _turnMotionAcivateAngle)
			{
			...
			if (!base.IsInterrupted)
			{
				...
				prevTime = Time.time;
				Vector3 vector = Maths.Make2D(funcTargetPos() - base.transform.position);
				if (!endAtReached || !(vector.magnitude < 100f))
				{
					float yaw = Maths.CalcYawWithTarget(funcTargetPos(), base.transform.position);
	...
	{
		float prevTime = Time.time;
		Vector2 vector = Util.ClientPositionToTilePosition(base.transform.position);
		Vector2 vector2 = Util.ClientPositionToTilePosition(funcTargetPos());
		List<PathFinderNode> list = null;
		List<Vector2> list2 = null;
		NavGrid.EnsureLoaded();
		if (NavGrid.Ready)
		{
			list2 = NavGrid.FindPathTiles(base.transform.position, funcTargetPos());
		}
		if (list2 == null)
		{
			list = _pathFinder.FindPath(new Point((int)vector.x, (int)vector.y), new Point((int)vector2.x, (int)vector2.y));
		}
		...
		if (list2 != null)
		{
			...
			{
				paths.Add(Util.TilePositionToClientPosition(list2[i], tileCenter: true));
			}
		...
		{
			bool flag = funcCheckWalk?.Invoke(wasLastMoveWalk) ?? false;
			bool flag2 = wasLastMoveWalk != flag;
			wasLastMoveWalk = flag;
			string text = ((!flag) ? runMotion : walkMotion);
			float moveSpeed = ((!flag) ? runSpeed : walkSpeed);
			float ang = Maths.CalcYawWithTarget(destPos, base.transform.position);
			if (!isMoving && Maths.DistanceAngDeg(ang, TargetAnimal.CurrentYaw) > _turnMotionAcivateAngle)
			{
				yield return StartCoroutine(CoTurnAndCrossFadeMotion(text, 0.1f));
				isMoving = true;
				prevTime = Time.time;
			}
			else if (!isMoving || flag2)
			{
				CrossFadeAndFitLocation(text, 0.1f);
				isMoving = true;
			}
			if (!base.IsInterrupted)
			{
				...
				{
					float yaw = Maths.CalcYawWithTarget(destPos, base.transform.position);
					TargetAnimal.TurnToYaw(yaw, bSnap: false);
					Vector3 vector3 = Maths.Make2D(destPos - base.transform.position);
					if (vector3.magnitude < 200f && paths.Count > 0)
					{
						destPos = paths[0];
						paths.RemoveAt(0);
					}
					Vector3 vector4 = vector3.normalized * moveSpeed;
					TargetAnimal.CurrentPosition += vector4 * num;
					yield return null;
					continue;
				}
				break;
