...
public class MiniGameDanceNote : MonoBehaviour
{
	...
	private IEnumerator FlyingSequence(float startTime, MiniGameDanceAsset.DanceNoteData data, UIWidget target, Action<float, MiniGameDanceNote, bool> destroyCallback)
	{
		...
		Vector3 spawnPos = target.transform.position + new Vector3(Mathf.Cos(f), Mathf.Sin(f), 0f) * 4f;
		base.transform.position = spawnPos;
		float reachAt = startTime + data.TimeKey;
		for (float elapsedTime = MiniGameDanceHelper.ElapsedTime; elapsedTime < reachAt; elapsedTime = MiniGameDanceHelper.ElapsedTime)
		{
			float num = (reachAt - elapsedTime) / data.TransitionTime;
			base.transform.position = Vector3.Lerp(spawnPos, target.transform.position, 1f - num);
			yield return null;
		}
		...
		{
			float num2 = (MiniGameDanceHelper.ElapsedTime - fadeSince) / 0.5f;
			base.transform.Translate(previousVelocity * MiniGameDanceHelper.DeltaTime);
			_arrowSprite.alpha = 1f - num2;
			yield return null;
		}
