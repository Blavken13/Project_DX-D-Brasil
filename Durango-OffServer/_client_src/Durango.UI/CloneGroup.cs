...
public class CloneGroup : UIBase
{
	...
	private void OnPostRaycast(UICamera.MouseOrTouch touch)
	{
		...
		if (_touch != null)
		{
			...
			{
				x = (touch.pos.x - pixelRect.x) / pixelRect.width,
				y = (touch.pos.y - pixelRect.y) / pixelRect.height
			};
			Vector2 vector2 = new Vector2
			{
				x = (vector.x - _cloneScreenRect.x) / _cloneScreenRect.width,
				y = (vector.y - _cloneScreenRect.y) / _cloneScreenRect.height
			};
			Vector2 vector3 = new Vector2
			{
				x = _originScreenRect.x + _originScreenRect.width * (vector2.x - _currentRatio),
				y = _originScreenRect.y + _originScreenRect.height * vector2.y
			};
			Vector2 vector4 = (touch.pos = new Vector2
			{
				x = pixelRect.x + pixelRect.width * vector3.x,
				y = pixelRect.y + pixelRect.height * vector3.y
			});
