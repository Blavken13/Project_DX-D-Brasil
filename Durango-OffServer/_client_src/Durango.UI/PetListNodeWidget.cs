...
public class PetListNodeWidget : MonoBehaviour, IScreenResizeReceiver
{
	...
	private void UpdateLayout()
	{
		if (_isInit)
		{
			...
			list.AddRange(_infoNodes.Select((PetListInfoNode node) => node.GetComponent<UIWidget>()));
			...
			{
				float num = UIUtility.WidgetsReposition(list, Vector3.right, localCenter, 0f, 0.5f);
				component.SetDimensions((int)num, _baseInfoNode.Widget.height + _titleWidget.height);
			}
			else
			{
				float num2 = UIUtility.WidgetsReposition(list, Vector3.down, localCenter, 0f, 0.5f);
