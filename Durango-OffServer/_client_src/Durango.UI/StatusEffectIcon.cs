...
public class StatusEffectIcon : UIWidget
{
	...
	private IEnumerator CoFadeOut()
	{
		...
		while (timer < 3f)
		{
			...
			float num = Mathf.Sqrt(timer / 3f);
			base.transform.localPosition = defaultPos + Vector3.down * 100f * num;
			SetAlpha(1f - num);
			yield return null;
		}
		IsPlayingEffect = false;
	...
	private IEnumerator CoFadeIn()
	{
		...
		{
			timer = Time.time - startTime;
			float num = timer / effectTime;
			switch (state)
			{
			case 0:
				base.transform.localPosition = defaultPos + Vector3.up * 60f * Mathf.Pow(num, 2f);
				SetAlpha(Mathf.Clamp01(num * 2f));
				break;
			case 1:
				base.transform.localPosition = defaultPos + Vector3.up * 10f * num;
				break;
			case 2:
				localPosition.x = Mathf.Lerp(defaultPos.x, _fadeInTargetPos.x, Mathf.Pow(num, 2f));
				localPosition.y = Mathf.Lerp(defaultPos.y, _fadeInTargetPos.y, Mathf.Pow(num, 2f));
				localPosition.z = 0f;
				base.transform.localPosition = localPosition;
				SetAlpha(Mathf.Pow(Mathf.Abs(num * 2f - 1f), 3f));
				break;
			}
