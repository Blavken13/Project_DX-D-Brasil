...
public class CommandButtonGroup : UIBase
{
	...
	private void OpenSucceed()
	{
		...
		foreach (KeyValuePair<string, RegionTemplate> item3 in SingletonDict<string, RegionTemplate>.Instance)
		{
			if (!item3.Value.Active)
			{
				...
				{
					hashSet.Add(item);
					panel.AddSeperatorButton("Disabled templates below");
				}
		...
		if ((bool)panelSkill)
		{
			...
			}).On(delegate(Info msg, PacketHeader header)
			{
				...
				{
					panelSkill.AddPushButton("Learn all skills", "scl all", disabled: false);
	...
	{
		Dictionary<string, bool> dictionary = msg.Flags ?? new Dictionary<string, bool>();
		OffServerLink.SetAdminFlags(dictionary);
		dictionary["ar"] = Durango.Utils.Singleton<Commands>.Instance().GetAttackRangeState();
		for (int i = 0; i < _panelContainers.Length; i++)
		{
			_panelContainers[i].RefreshToggleButtonSelectStates(dictionary);
		}
	}
