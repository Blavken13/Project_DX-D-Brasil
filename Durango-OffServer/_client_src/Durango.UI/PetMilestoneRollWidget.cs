...
public class PetMilestoneRollWidget : MonoBehaviour, IUIInitializable
{
	...
	private IEnumerator RollAnimationCoroutine(float duration)
	{
		...
		while (true)
		{
			float num = Mathf.Clamp01((Time.time - start) / (end - start));
			...
			for (int i = 0; i < _rollItems.Count; i++)
			{
				...
				if (num2 < rollItem.Begin)
				{
					break;
				}
				float begin = rollItem.Begin;
				float num3 = Mathf.Min(rollItem.End, num2);
				_terms.GetOrAdd(i).DrawArc(90f - num3, 90f - begin, 0f - (ringSize + ringPadding * 2f), 0f, rollItem.BgColor, 0f - (ringSize * rollItem.FillRatio + ringPadding), 0f - ringPadding, rollItem.Color);
				if (!(num2 < rollItem.End))
				{
					UISprite orAdd = _grades.GetOrAdd(i);
					orAdd.color = rollItem.Color;
					if (rollItem.BgColor.a <= 0f)
					{
						orAdd.alpha = 0f;
					}
					float num4 = Mathf.Lerp(rollItem.Begin, rollItem.End, 0.5f);
					orAdd.transform.localEulerAngles = new Vector3(0f, 0f, 0f - num4);
					num4 = 90f - num4;
					Vector3 vector = new Vector3(Mathf.Cos(num4 * ((float)Math.PI / 180f)), Mathf.Sin(num4 * ((float)Math.PI / 180f)));
					float num5 = (float)Mathf.Min(_widget.width, _widget.height) * 0.5f;
					orAdd.transform.localPosition = (num5 - (ringSize + ringPadding * 2f) * 0.8f) * vector;
					orAdd.transform.localScale = Vector3.one * (ringSize * 0.1f) / orAdd.width;
					UISprite orAdd2 = _termSeparators.GetOrAdd(i);
					RollItem rollItem2 = _rollItems[(i + 1) % _rollItems.Count];
					float num6 = Mathf.Lerp(rollItem.End, (!(rollItem2.Begin < rollItem.End)) ? rollItem2.Begin : (rollItem2.Begin + 360f), 0.5f);
					orAdd2.transform.localEulerAngles = new Vector3(0f, 0f, 0f - num6);
					num6 = 90f - num6;
					Vector3 vector2 = new Vector3(Mathf.Cos(num6 * ((float)Math.PI / 180f)), Mathf.Sin(num6 * ((float)Math.PI / 180f)));
					float num7 = (float)Mathf.Min(_widget.width, _widget.height) * 0.5f;
					orAdd2.transform.localPosition = num7 * vector2;
