...
public class WorldMapGroup : UIBase
{
				...
				new ButtonInfo[1]
				{
					new ButtonInfo
					{
						...
						ValidFunc = delegate
						{
							...
							return ((uint)(role - 3) <= 3u || role == Role.Personal) ? true : false;
						}
					}
				},
				null,
	...
	private void OnClickWorldMap(GameObject obj)
	{
		...
		UICamera.MouseOrTouch currentTouch2 = UICamera.currentTouch;
		if (currentTouch2 != null)
		{
			Vector2 tilePosition = Durango.Utils.Singleton<MapContext>.Instance().ScreenPosToTilePos(currentTouch2.pos);
			if (!(tilePosition.x < 0f) && !(tilePosition.y < 0f) && !(tilePosition.x >= (float)TerrainMeta.TileCount) && !(tilePosition.y >= (float)TerrainMeta.TileCount) && !(PlayerBehavior.LocalPlayer == null) && Durango.Utils.Singleton<PlayerController>.HasInstance())
			{
				Vector3 pos = Durango.Terrain.Util.TilePositionToClientPosition(tilePosition, tileCenter: true);
				Close();
				Durango.Utils.Singleton<PlayerController>.Instance().MoveToPosition(pos, null, 10f, completeIfBlocked: false, showMoveCursor: true);
			}
		}
	}
	...
	private static void HideIndicatorsWithoutWarpTargets(MapIndicators inds)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (indicatorType == IndicatorType.LocalPlayer || indicatorType == IndicatorType.Dock || (uint)(indicatorType - 9) <= 1u)
			{
