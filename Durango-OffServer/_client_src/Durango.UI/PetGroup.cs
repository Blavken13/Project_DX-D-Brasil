...
public class PetGroup : UIBase
{
	...
	private void SelectTab(int index)
	{
		...
		if (KUtility.GetSize(GetPets(petOwnType)) > 0)
		{
	...
	private void OnPetActionClick(PetInfoWidget.PetAction action, Messages.Pet pet)
	{
		...
		case PetInfoWidget.PetAction.PutInToStorage:
		{
			...
			{
				PetManager.GrazePets((pets2 != null) ? pets2.Select((Messages.Pet p) => p.EntityId).Concat(new string[1] { pet.EntityId }).ToArray() : new string[1] { pet.EntityId }, RefreshPetList);
				break;
			}
			UIManager.Popup.Tooltip<SelectPetPopup>().SetTitle(T._("동물 교체")).SetInfo(T._("방목 중인 동물이 가득 찼습니다.\n교체할 동물을 선택하세요."))
				.SetList(pets2)
				.SetOnConfirm(delegate(Messages.Pet selected)
				{
					PetManager.GrazePets((pets2 != null) ? (from p in pets2
						select p.EntityId into id
						where id != selected.EntityId
						select id).Concat(new string[1] { pet.EntityId }).ToArray() : new string[1] { pet.EntityId }, RefreshPetList);
				})
		...
		case PetInfoWidget.PetAction.TakeOutFromStorage:
		{
			...
			UIManager.Popup.Tooltip<SelectPetPopup>().SetTitle(T._("동물 교체")).SetInfo(T._("관리 중인 동물이 가득 찼습니다.\n교체할 동물을 선택하세요."))
				.SetList(list)
				.SetOnConfirm(delegate(Messages.Pet selected)
				{
					PetManager.GrazePets((from p in pets
						select p.EntityId into id
						where id != pet.EntityId
						select id).Concat(new string[1] { selected.EntityId }).ToArray(), RefreshPetList);
				})
				.SetConfirmButtonText(T._("교체하기"))
				.Show();
			break;
	...
	private void ShowPetSelectorPopup(GameObject _)
	{
		...
		List<Messages.Pet> list = (from pet in array.Where(delegate(Messages.Pet pet)
			{
