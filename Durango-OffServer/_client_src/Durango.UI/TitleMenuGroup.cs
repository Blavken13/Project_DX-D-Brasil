...
public class TitleMenuGroup : MonoBehaviour
{
	...
	private State CurState
	{
		...
		set
		{
			...
			case State.Initial:
				...
				OffServerLink.FetchServers(delegate
				{
					try
					{
						TextAsset textAsset2 = Resources.Load("offline/clusters") as TextAsset;
						if (!(textAsset2 == null) && UserControl.TryUpdateClusters(textAsset2.text))
						{
							UserControl.OnClustersRefreshed();
						}
					}
					catch
					{
	...
	private IEnumerator CoPollServerStatus()
	{
		while (true)
		{
			string text = ((!string.IsNullOrEmpty(GameManager.GatewayUrl)) ? GameManager.GatewayUrl : OffServerLink.Gateway);
			...
			try
			{
				HTTPRequest hTTPRequest = new HTTPRequest(new Uri(text.TrimEnd('/') + "/status"), HTTPMethods.Get, delegate(HTTPRequest req, HTTPResponse resp)
				{
					try
					{
						if (resp != null && resp.IsSuccess)
						{
							string text2 = resp.DataAsText ?? string.Empty;
							Match match = Regex.Match(text2, "\"online\"\\s*:\\s*(\\d+)");
							up = text2.Contains("\"ok\":true") || text2.Contains("\"ok\": true");
							if (match.Success)
							{
								int.TryParse(match.Groups[1].Value, out online);
							}
						}
					}
					catch
			...
			OffServerLink.SetServerStatus(up, online);
			if (UserControl != null)
			{
				UserControl.RefreshVersionInfo();
			}
	...
	{
		if (!(UserControl == null) && (IsDataLoadState(_curState) || _curState == State.Connecting || _curState == State.Welcome || _curState == State.Loading))
		{
			UserControl.SetExplainLabel(OffServerLink.DownloadProgressText());
		}
	}
	...
	private void OnRequestSucceed(string response)
	{
		...
		case State.IdleInHardcapPosition:
		{
			...
			else
			{
				...
				string text5 = T._("확인 버튼을 터치하면 게임이 종료됩니다.");
