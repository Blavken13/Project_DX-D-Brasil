...
public class DevelopmentGroup : UIBase
{
	...
	{
		bool isAdmin = OffServerLink.IsAdmin;
		if (isAdmin == _adminShown)
		{
			return;
		}
		_adminShown = isAdmin;
		_container.gameObject.SetActive(isAdmin);
		if (!isAdmin)
		{
			_stats.SetActive(value: false);
			CommandButtonGroup commandButtonGroup = UIManager.FindScript<CommandButtonGroup>();
			if (commandButtonGroup != null && commandButtonGroup.IsOpened)
			{
