...
public class WarehouseTabSelector : TooltipBase
{
	...
	private void UpdateData()
	{
		...
		if (_hasMyInven)
		{
			...
			bool flag = (playerInvenTab.GetComponent<Selectable>().Disabled = num2 < _requireSize);
			playerInvenTab.Set(value: string.Format((!flag) ? "<em>{0}</em>/{1}" : "{0}/{1}", num, capacity), key: T._("가방"));
		}
		else
		{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (_exceptList == null || Array.IndexOf(_exceptList, keyValuePair.Key) == -1)
			{
				...
				bool flag = (component.GetComponent<Selectable>().Disabled = num3 < _requireSize);
				component.Set(value: string.Format((!flag) ? "<em>{0}</em>/{1}" : "{0}/{1}", keyValuePair.Value, Singleton<Constants>.Instance.Warehouse.SectionSize), key: keyValuePair.Key);
