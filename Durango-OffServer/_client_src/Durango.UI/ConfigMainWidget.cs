...
public class ConfigMainWidget : MonoBehaviour
{
		...
		foreach (Setting item in list)
		{
			...
			try
			{
				...
				case SettingType.TextInput:
					...
					{
						settingItem = AddTextInput(valueSetting5);
				...
				case SettingType.TextLabel:
					if (item is LabelSetting labelSetting)
					{
						...
						if (item.Key == "private_number")
						{
							MakeAccountRowClickable(settingItem);
						}
					}
					break;
				...
				case SettingType.Switch:
					...
					{
						settingItem = AddSwitch(valueSetting4);
			...
			{
				UnityEngine.Debug.LogWarning("[OffServer] สร\u0e49างแถวต\u0e31\u0e49งค\u0e48า '" + item.Key + "' ไม\u0e48ได\u0e49 ข\u0e49ามไป: " + ex.Message);
				settingItem = null;
			}
			if (settingItem == null)
			{
	...
	protected virtual SettingItem AddDropdown(ValueSetting op, string[] dropdownOptions, bool isCloseOnButtonClick, bool isCustom)
	{
		...
		if (_dropdownBase == null || _dropdownPool.BaseObject == null)
		{
			return AddToggle(op, dropdownOptions);
		}
		SettingItem settingItem = MakeItemWithLabelKey(op);
	...
	private void DoLocalize(SettingItem setting)
	{
		...
		case SettingType.TextLabel:
			if (setting.Key == "private_number")
			{
				UnityEngine.Debug.Log("[OffServer] settings ▸ Account Number = '" + (setting.Contents as UILabel).text + "'");
			}
			break;
		case SettingType.Button:
		case SettingType.TinyButton:
	...
	private void MakeAccountRowClickable(SettingItem settingItem)
	{
		UILabel uILabel = settingItem.Contents as UILabel;
		if (!(uILabel == null))
		{
			...
			GameObject gameObject = uILabel.gameObject;
			...
			boxCollider.center = new Vector3(-300f + localSize.x / 2f + 300f, 0f, 0f);
			boxCollider.size = new Vector3(localSize.x + 900f, localSize.y + 24f, 20f);
			UIEventListener.Get(gameObject).onClick = delegate
			{
				UnityEngine.Debug.Log("[OffServer] Account Number row clicked");
