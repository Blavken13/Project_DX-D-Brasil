...
public class ExpectResultWidget : UIWidget
{
	...
	protected override void OnStart()
	{
		...
		_ = _helpTouchBox == null;
		_ = _previewWidget == null;
		_ = _bonusItemWidget == null;
		_ = _quantitySelector == null;
		...
		if (_quantitySelector != null)
		{
			IntSelector quantitySelector2 = _quantitySelector;
			quantitySelector2.ValueChanged = (Action)Delegate.Combine(quantitySelector2.ValueChanged, new Action(OnQuantityChanged));
		}
	}
	...
	public void Set(CraftSlotContainer slotContainer)
	{
		...
		if (_craft.TechSupportBaseSlotInfo == null)
		{
			...
			try
			{
				max = Mathf.Max(1, _craft.CalcMaxQuantity());
			}
			catch (Exception)
			{
			}
		...
		else if (_quantitySelector != null)
		{
			_quantitySelector.gameObject.SetActive(value: false);
		}
		BonusPrototypes[] array = ((slotContainer.Recipe != null) ? SingletonDict<string, BonusPrototypes[]>.Get(slotContainer.Recipe.Id) : null);
		if (_bonusItemWidget != null)
		{
	...
	public void Set(BuildSlotContainer slotContainer)
	{
		...
		if (_remodelingWidget != null)
		{
			_remodelingWidget.gameObject.SetActive(slotContainer.Blueprint != null && slotContainer.Blueprint.IsRemodeling);
		}
		if (_quantitySelector != null)
		{
			_quantitySelector.gameObject.SetActive(value: false);
