...
public class ChattingGroup_PC : ChattingGroupBase
{
	...
	public void Show(bool isFocus = true)
	{
		...
		for (int j = 0; j < hideOnShrinks.Length; j++)
		{
			hideOnShrinks[j].alpha = 1f;
		}
		BoxCollider[] hidableColliders = _hidableColliders;
		for (int k = 0; k < hidableColliders.Length; k++)
		{
			hidableColliders[k].enabled = true;
		}
	...
	public void Shrink()
	{
		...
		for (int k = 0; k < hidableColliders.Length; k++)
		{
			hidableColliders[k].enabled = false;
		}
