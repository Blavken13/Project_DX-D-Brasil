...
public class MiniGameDanceGroup : UIBase
{
	...
	private IEnumerator RythmnGameSequence(MiniGameStatus status)
	{
		...
		float elapsedTime = MiniGameDanceHelper.ElapsedTime;
		while (elapsedTime < _status.StartTime + _status.GetDuration() && !(MiniGameDanceHelper.ElapsedTime - _status.StartTime > _status.GetDuration()))
		{
			while (true)
			{
