	...
	private void ShowItemInfo(ItemIconWidget itemIcon)
	{
		...
		if (itemIcon.IconMode != ItemIconWidget.Mode.Enabled && _selectedTab == 0 && _other != null)
		{
			if (KUtility.GetSize(_other.StorableTags) > 0)
			{
				...
				{
					if (SingletonDict<string, Yaml.Tag>.TryGetValue(_other.StorableTags[i], out var value))
					{
						list.Add(value.Name);
					}
				}
			...
			else if (KUtility.GetSize(_other.UnstorableTags) > 0)
			{
				...
				{
					if (SingletonDict<string, Yaml.Tag>.TryGetValue(_other.UnstorableTags[j], out var value2))
					{
