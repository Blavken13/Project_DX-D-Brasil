...
public class PortraitMenuList : MenuListWidgetBase, IMenuList
{
	...
	public void Refresh()
	{
		Init();
		_menuList.BeginLoad();
		foreach (MenuType item in MenuContainer.FirstDepthMenus.Where((MenuType menu) => (!MenuContainer.HasChildren(menu)) ? GameSystem<MenuSystem>.Instance().IsEnabled(menu) : MenuContainer.GetChildren(menu).Any((MenuType c) => GameSystem<MenuSystem>.Instance().IsEnabled(c))))
		{
