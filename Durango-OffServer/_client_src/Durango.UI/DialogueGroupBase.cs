...
public class DialogueGroupBase : UIBase
{
	...
	public Dictionary<string, object> TestBridgeInfo()
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>();
		dictionary["open"] = base.IsOpened;
		dictionary["queued"] = _contexts.Count;
		if (Current == null)
		{
			return dictionary;
		}
		dictionary["type"] = Current.Type.ToString();
		dictionary["name"] = Current.Name.Text;
		dictionary["message"] = TestBridge.StripTags(Current.Message.Text);
		dictionary["event"] = ((Current.Guide != null) ? Current.Guide.Name : null);
		dictionary["chapter"] = Current.ChapterTitle;
		dictionary["duration"] = Current.Duration;
		dictionary["readyToNext"] = _readyToNext;
		if (Current.Quiz != null)
		{
			List<object> list = new List<object>();
			if (Current.Quiz.Choices != null)
			{
				for (int i = 0; i < Current.Quiz.Choices.Length; i++)
				{
					list.Add(Current.Quiz.Choices[i]);
				}
			}
			dictionary["choices"] = list;
		}
		return dictionary;
	}

	public string TestBridgeCurrentType()
	{
		if (Current == null)
		{
			return null;
	...
	public void TestBridgePress(bool pressed)
	{
		OnPressDialogue(pressed);
	}

	public bool TestBridgeAnswerQuiz(int index)
	{
		if (Current == null || Current.Type != Type.Quiz || Current.Quiz == null || ChoicePool == null)
		{
			return false;
		}
		if (index < 0 || index >= Math.Min(3, KUtility.GetSize(Current.Quiz.Choices)))
		{
			return false;
		}
		ChoiceButton choiceButton = ChoicePool.Get<ChoiceButton>(index);
		if (choiceButton == null || choiceButton.Clicked == null)
		{
			return false;
		}
