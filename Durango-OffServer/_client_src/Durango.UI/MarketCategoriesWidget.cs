...
public class MarketCategoriesWidget : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			GameObject obj = nodes[num2 + 2];
			Category category2 = _categoryList[num2];
			obj.transform.Find("name").GetComponent<UILabel>().text = category2.MainCategory.Name;
			obj.transform.Find("icon").GetComponent<UISprite>().spriteName = category2.MainCategory.Icon;
		}
