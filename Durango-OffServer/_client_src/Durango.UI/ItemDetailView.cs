...
public class ItemDetailView : KWidgetScrollView
{
	...
	public void Set([NotNull] ItemData itemData, bool enableRecipeLink)
	{
		...
		IEnumerable<TagData> tags = ((itemData.ReformSlots.Count <= 0) ? itemData.Tags : EnumerateAdjustedItemTags(itemData.Tags, itemData.ReformSlots));
	...
	private static IEnumerable<TagData> EnumerateAdjustedItemTags(IEnumerable<TagData> itemTags, IEnumerable<ReformSlot> reformSlots)
	{
		...
		foreach (TagData itemTag in itemTags)
		{
			if (_reformSlotTagLevels.TryGetValue(itemTag.Id, out var value))
			{
				if (itemTag.Level - value > 0)
				{
					yield return TagData.Create(itemTag.Id, itemTag.Level - value);
				}
			}
			else
			{
