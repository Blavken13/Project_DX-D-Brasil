...
public class PartyGroup : UIBase
{
	...
	private void Start()
	{
		...
		electLeader.Clicked = (Action)Delegate.Combine(electLeader.Clicked, (Action)delegate
		{
			...
			{
				_scrollView.Nodes[i].GetComponent<PartyPlayerInfoWidget>().ToggleElectLeader(flag);
			}
		});
	...
	private static void InviteParty()
	{
		...
		UIManager.FindScript<PlayerSearchGroup>().OpenForMultiple(num, T._("파티 초대"), list, delegate(IList<string> list2)
		{
			if (list2 == null)
			{
				return;
