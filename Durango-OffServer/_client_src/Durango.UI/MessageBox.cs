...
public class MessageBox : UIBase
{
	...
	public string TestBridgeText()
	{
		if (!IsShow)
		{
			return null;
		}
		string text = ((_mainTextLabel != null) ? _mainTextLabel.text : string.Empty);
		if (_subTextLabel != null && !string.IsNullOrEmpty(_subTextLabel.text))
		{
			text = text + "\n" + _subTextLabel.text;
		}
		return TestBridge.StripTags(text);
	}

	public bool TestBridgeChoose(bool ok)
	{
		if (!IsShow || _buttons == null || _buttons.Count == 0)
		{
			return false;
		}
		int index = ((!ok) ? Math.Min(1, _buttons.Count - 1) : 0);
		Selectable.Current = _buttons[index];
		OnButtonClick();
		return true;
	}
