...
public class ItemIconTex : UITexture
{
	...
	public override void OnFill(UIGeometry.Arguments arguments)
	{
		...
		Vector4 drawingArea = new Vector4
		{
			x = Mathf.Lerp(vector.x, vector.z, _iconRect.x),
			z = Mathf.Lerp(vector.x, vector.z, _iconRect.z),
			y = Mathf.Lerp(vector.y, vector.w, _iconRect.y),
			w = Mathf.Lerp(vector.y, vector.w, _iconRect.w)
		};
