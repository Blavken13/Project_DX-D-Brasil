...
public class AnimalBehavior : CharacterBehavior, IMotionPlayable, IAnimationEventPlayable, IBoneMergedObserver
{
	...
	protected new void Start()
	{
		...
		rendererProxy.Add(renderers2, isAnimal: true);
	...
	private IEnumerator CoChangeTensionColor(Color color, float duration = 2f)
	{
		...
		while (true)
		{
			float num = Mathf.Clamp01((Time.time - beginTime) / duration);
			_tensionColor = Color.Lerp(beginColor, color, num);
			if (!(num >= 1f))
			{
				yield return null;
				continue;
