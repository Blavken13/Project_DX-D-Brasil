...
public class CloudUpdater : MonoBehaviour
{
	...
	private IEnumerator CoProcessCloud()
	{
		while (true)
		{
			Vector3 randomPos = GetRandomPos(PlayerBehavior.LocalPlayer.CurrentPosition, 5000f);
			_cloud.transform.rotation = Quaternion.Euler(new Vector3(90f, UnityEngine.Random.Range(0, 360), 0f));
			_cloud.transform.position = randomPos;
			float num = _initialScale * (1f + UnityEngine.Random.Range(-0.2f, 0.2f));
			_cloud.transform.localScale = new Vector3(num, num, 0f);
	...
	private IEnumerator CoFlowFading(bool appear)
	{
		...
		{
			MoveCloud();
			float num = _fadeSpeed * Time.deltaTime;
			alpha = ((!appear) ? (alpha + num) : (alpha - num));
