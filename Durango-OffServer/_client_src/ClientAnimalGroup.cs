...
public class ClientAnimalGroup : MonoBehaviour
{
	...
	public IEnumerator CoPlay()
	{
		...
		{
			float num = Time.time - _spawnBeginTime;
			if (_cachedPrefab != null)
			{
				for (int num2 = _spawnList.Count - 1; num2 >= 0; num2--)
				{
					SpawnInfo spawnInfo = _spawnList[num2];
					if (!(spawnInfo.SpawnTime > num))
					{
						GameObject obj = UnityEngine.Object.Instantiate(_cachedPrefab);
