...
public static class Json
{
	...
	public static string OverridePath(string fileName)
	{
		try
		{
			string text = Path.Combine(Path.Combine(Path.GetDirectoryName(Application.dataPath) ?? ".", "override"), fileName.Replace('/', Path.DirectorySeparatorChar) + ".bytes");
	...
	{
		string text = OverridePath(fileName);
		if (text != null)
		{
			return Read<T>(File.ReadAllText(text));
		}
