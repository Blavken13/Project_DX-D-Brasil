...
public class PlayerController : Durango.Utils.Singleton<PlayerController>
{
	...
	public void TestBridgeMoveTo(Vector3 pos, GameObject target, Action onComplete, float distance = 10f)
	{
		if (!_manualPickingMove)
		{
			_manualPickingMove = true;
		...
		if (target != null)
		{
			MoveToTarget(target, onComplete2, distance, completeIfBlocked: true);
		}
		else
		{
			MoveToPosition(pos, onComplete2, distance, completeIfBlocked: true, showMoveCursor: true);
		}
	}
