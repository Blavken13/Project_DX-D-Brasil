...
public class SocialSystem : GameSystem<SocialSystem>
{
	...
	{
		TestBridge.Record("chat", (string.IsNullOrEmpty(speakerName) ? "" : (speakerName + ": ")) + chatText);
		if (remainColor)
		{
	...
	public void Say(string message, bool isDictation = false)
	{
		...
		if (!string.IsNullOrEmpty(message))
		{
			...
			((GameManager.ClusterMode != Mode.Online) ? Connections.Frontend : Connections.Radiotower).Send(msg);
			AddContinuousChatTime(channelType);
		}
	}
