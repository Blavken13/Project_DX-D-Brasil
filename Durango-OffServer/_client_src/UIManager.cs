...
public class UIManager : Singleton<UIManager>, IUriInvokable
{
	...
	public IEnumerable<string> CollectUri()
	{
		...
		foreach (KeyValuePair<string, IUriInvokable> invoker in linkedPrefabs.GetUriInvokers())
		{
			foreach (string item in invoker.Value.CollectUri())
			{
	...
	private IEnumerator CoUpdateScreenSize()
	{
		...
		int width;
		int height;
		while (!Platform.Instance.GetScreenResolution(IsPortraitScreen, out width, out height))
		{
			yield return null;
		}
	...
	{
		TestBridge.Record("system", comment);
		if (Singleton<UIManager>.HasInstance())
		{
