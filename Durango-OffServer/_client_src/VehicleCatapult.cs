...
public class VehicleCatapult : VehicleProp
{
	...
	private IEnumerator CoAttack(VehicleProjectileFired msg)
	{
		...
		float y = Maths.NormalizeAngDeg(Maths.CalcYawWithTarget(targetPos, artifact.Center) + _yawBias);
		Quaternion destRot = Quaternion.Euler(0f, y, 0f);
		...
		{
			double num = (GetServerTime() - beginAt) / (msg.PrepareAnimAt - beginAt);
			_bodyTransform.localRotation = Quaternion.Lerp(origRot, destRot, Mathf.Clamp01((float)num));
			if (num >= 1.0)
			{
				break;
			}
		...
		if ((bool)animatingProp)
		{
			...
			float num2 = (float)(msg.DmgAt - GetServerTime());
			float speed = Maths.Make2D(targetPos - _attachmentProjectile.position).magnitude / num2;
			ProjectileController.ModifyProjectileSpeed(speed);
