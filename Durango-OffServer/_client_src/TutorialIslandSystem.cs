...
public class TutorialIslandSystem : GameSystem<TutorialIslandSystem>
{
	...
	public Point2? TutorialBoatTile
	{
		get
		{
			...
			return _tutorialBoat.WorldTile;
		}
	}
	...
	public Dictionary<string, object> TestBridgeInfo()
	{
		...
		dictionary["hasBoat"] = _tutorialBoat != null;
		dictionary["hasSession"] = _hasSession;
		if (_tutorialBoat != null)
		{
			dictionary["boatEntityId"] = _tutorialBoat.EntityId;
			dictionary["boatTile"] = _tutorialBoat.WorldTile.x + "," + _tutorialBoat.WorldTile.y;
		}
		if (_hasSession)
		{
			dictionary["sessionId"] = TutorialSession.SessionId;
			Dictionary<string, object> dictionary2 = new Dictionary<string, object>();
			if (TutorialSession.Materials != null)
			{
				foreach (KeyValuePair<string, int> material in TutorialSession.Materials)
				{
					dictionary2[material.Key] = material.Value;
				}
			}
			dictionary["materials"] = dictionary2;
		}
		return dictionary;
	}

	public string TestBridgePutAllMaterials()
	{
		if (_tutorialBoat == null)
		{
		...
		_boatSlots.QuickFill();
		Dictionary<string, string[]> dictionary = _boatSlots.CreateFirstMaterialsDictionary(canFinished: false);
		int num = 0;
		if (dictionary != null)
		{
			foreach (KeyValuePair<string, string[]> item in dictionary)
			{
				num += ((item.Value != null) ? item.Value.Length : 0);
			}
		}
		if (num == 0)
		{
	...
	public string TestBridgeDepart()
	{
		if (_tutorialBoat == null)
		{
			return "ย\u0e31งไม\u0e48เห\u0e47นแพ (AppearTutorialBoat)";
	...
	{
		DepartTutorialFor msg = new DepartTutorialFor
		{
