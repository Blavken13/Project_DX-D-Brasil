...
public class TerrainChunk_Mobile : TerrainChunkBase
{
	...
	protected override IEnumerator LoadNaturals(IList<NaturalInfo> naturalData)
	{
		...
		for (int i = 0; i < naturalData.Count; i++)
		{
			...
			Point2 point = FromWorldTile(new Point2(naturalData[i].X, naturalData[i].Y));
			TileObject tileObject = base.StaticObjectChunk.GetTileObject(point);
			if (tileObject != null)
			{
				FillShrubCollisionToGrid(point, tileObject.NaturalObject);
			}
		}
